...
{
    fn evaluate_sample<'a, Elem: 'a, Sample: IndexCopy<Index, Output = Elem>>(
        &self,
        sample: &'a Sample,
    ) -> Bias
    where
        ...
        if let Some(ho) = &self.higher_order {
            for (contribs, bias) in ho.iter_contrib() {
                value += *bias
                    * contribs
                        .iter()
                        .fold(Bias::one(), |acc, x| sample.index_copy(*x) * acc);
            }
        }
        value
    }
    ...
    fn evaluate_sampleset<
        'a,
        Elem: 'a,
        Sample: IndexCopy<Index, Output = Elem> + 'a,
