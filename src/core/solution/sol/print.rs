...
use super::{Column, PrintLayout, ShowMetadata};
...
impl Solution {
    ...
    fn print_col_layout(
        ...
        if matches!(show_metadata, ShowMetadata::Before | ShowMetadata::After) {
            ...
            for (row_idx, feasible) in maybe_sorted_by_idxs(&self.feasible, &idxs)[..n_rows]
                .iter()
                .enumerate()
            {
            ...
            let mut col_width = 3;
            for raw in maybe_sorted_by_idxs(&self.raw_energies, &idxs)[..n_rows].iter() {
                let s = match raw {
                    None => String::from("?"),
            ...
            col_width = 3;
            for obj in maybe_sorted_by_idxs(&self.obj_values, &idxs)[..n_rows].iter() {
                let s = match obj {
                    None => String::from("?"),
                    Some(bias) => Self::format_bias(*bias, max_col_size),
                };
    ...
    fn print_row_layout(
        ...
        if matches!(show_metadata, ShowMetadata::Before | ShowMetadata::After) {
            ...
            for (j, feasible) in maybe_sorted_by_idxs(&self.feasible, &idxs)
                .iter()
                .enumerate()
            {
                let s = match feasible {
            ...
            width_reached = 0;
            for (j, obj) in maybe_sorted_by_idxs(&self.obj_values, &idxs)
                .iter()
                .enumerate()
            {
                let s = match obj {
    ...
    fn get_sample_indices_sorted(&self) -> Vec<usize> {
        ...
        idxs.sort_by(|&idx1, &idx2| 'res: {
            let feas = self
                ...
                .as_ref()
                .map(|f| f[idx2])
                .cmp(&self.feasible.as_ref().map(|f| f[idx1]));
            if feas != Ordering::Equal {
                break 'res feas;
            }
            let obj = Self::cmp_bias(
                self.obj_values.as_ref().map(|o| o[idx2]).map(|b| b * sense),
                self.obj_values.as_ref().map(|o| o[idx1]).map(|b| b * sense),
            );
