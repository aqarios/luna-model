...
class DebugTransformation(TransformationPass[DebugArtifact]):
    ...
    def forward(self, model: Model, ctx: PassContext) -> tuple[Model, DebugArtifact]:
        max_bias = ctx.require_analysis(MaxBiasAnalysis.key())
...
class MaxBiasAnalysis(AnalysisPass[MaxBias]):
    PROVIDES = "max-bias-key"

    def name(self) -> str:
        return "max-bias"

    def run(self, model: Model, ctx: PassContext) -> MaxBias:
        _ = ctx
        max_val = 0.0
...
pm = PassManager([MaxBiasAnalysis(), DebugTransformation()])
out = pm.run(model)
print(out)
# print("---------")
# print("OUT MODEL")
