...
class BuiltinComposite(Generic[Artifact, Result]):
    ...
    def name(self) -> str:
        ...
        """
        sup = cast("_BuiltinCompositeSuper[Artifact, Result]", super())
        return sup.name()
    ...
    def forward(self, model: Model, ctx: PassContext) -> tuple[Model, Artifact, Result]:
        ...
        """
        sup = cast("_BuiltinCompositeSuper[Artifact, Result]", super())
        result: tuple[PyModel, Artifact, Result] = sup.forward(model._m, ctx._c)
        model, artifact, res = result
        return Model._from_pym(model), artifact, res
