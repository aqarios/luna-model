...
impl PyPassManager {
    #[new]
    fn new(py: Python, passes: Option<Vec<Py<PyPass>>>) -> PyResult<Self> {
        Ok(PyPassManager {
            pm: match passes {
                Some(steps) => PassManager::from_steps(
                    steps.into_iter().map(|p| p.borrow(py).to_step()).collect(),
                ),
    ...
    fn add(&mut self, py: Python, pass: Py<PyPass>) {
        let step = pass.borrow(py).to_step();
