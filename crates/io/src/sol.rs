...
impl CustomFormat<FormatOpt> for Solution {
    fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>, format_type: &FormatOpt) -> std::fmt::Result {
        match format_type {
            ...
            #[cfg(feature = "py")]
            FormatOpt::Py => write!(fmt, "{}", solpystr(&self, &PySolFormatOpts::default())),
            #[cfg(feature = "py")]
            FormatOpt::PySol(opts) => write!(fmt, "{}", solpystr(&self, opts)),
        }
    }
...
fn solpystr(sol: &Solution, opts: &PySolFormatOpts) -> String {
    match opts.layout {
