...
from aqmodels import Variable, Vtype, Model, Solution, Environment, Sense
...
def model_samples():
    ...
    samples = [
        ...
        {b1: 1, b2: 1, s1: -1, s2: 1, i1: 0, i2: 12, r1: -23.1, r2: 0.001},
    ]
...
def test_v0_v1_equality_sol_eval_timing(
    ...
    model, samples = model_samples
    sol = Solution.from_dicts(samples, model=model, counts=[2, 1, 1])
    with open(path / "sol_eval_timing", "rb") as f:
        ...
        assert reconstructed.runtime.qpu == 0.0142
